Template.projectPanelHeading.helpers({
  ...
  editing_datedue: function () {
    ...
    editing_projectnotes: function () {
    return Session.get('editing_projectnotes');
  },
  customer: function () {
...
Template.projectPanelHeading.events({
  ...
  'click .projectNameEdit': function (evt, tmpl) {
    ...
    'click .projectNotesEdit': function (evt, tmpl) {
    Session.set('editing_projectnotes', true);
    Meteor.setTimeout(function () {
      $('.form-control.projectNotes').focus().select();
    }, 250);
  },
         ...
         onSelect: function (dateText) {
          ...
          Session.set('editing_datedue', false);
             ...
             onChangeDateTime:function(dp,$input){
                 ...
                 Meteor.call('updateProjectDate', Session.get('active_project'), $input.val());
          Session.set('editing_datedue', false);
  ...
  'keyup .projectName': function (event, tmpl) {
    ...
    'keyup .projectNotes': function (event, tmpl) {
    if (event.which === 27 || event.which === 13) {
      event.preventDefault();
      var ele = tmpl.find('.projectNotes');
      Meteor.call('updateProjectNotes', this._id, ele.value);
      Session.set('editing_projectnotes', false);
    }
  }
});
